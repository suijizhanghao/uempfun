...
generate_systemd_conf() {
    ...
    local -r service_file="${systemd_units_dir}/cib.${service_name}.service"
    # Default values
...
[Unit]
Description=Cib service for ${name}
# Starting/stopping the main cib service should cause the same effect for this service
PartOf=cib.service
...
[Install]
# Enabling/disabling the main cib service should cause the same effect for this service
WantedBy=cib.service
EOF
}
